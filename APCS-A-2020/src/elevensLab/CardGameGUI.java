...
public class CardGameGUI extends JFrame implements ActionListener {
	...
	public CardGameGUI(Board gameBoard) {
		// initialize board, selected cards, totalWins, and totalGames
		board = gameBoard;
		...
		for (int i = 0; i < cardCoords.length; i++) {
			...
			// if this is the fourth card in the row
			if (i % 5 == 4) {
				// x & y adjustments
				x = LAYOUT_LEFT;
				y += LAYOUT_HEIGHT_INC;
			}
			// otherwise, x adjustment
			else x += LAYOUT_WIDTH_INC;
		}
	 ...
	 */
	public void repaint() {
		// loop over all spots on the board
		for (int k = 0; k < board.size(); k++) {
			// grab the image file of this card
	...
	private String imageFileName(Card c, boolean isSelected) {
		// if this Card is null use the null image
		if (c == null) return "cards/back1.GIF";
		// set up the file name
		String str = "cards/" + c.getRank() + c.getSuit();
		// add an S if selected
		if (isSelected) str += "S";
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(replaceButton)) {
			...
			// loop over all card spots
			for (int k = 0; k < board.size(); k++) 
				// if this card is selected add it to selection
				if (selections[k]) selection.add(k);
			...
			// reset selections
			for (int k = 0; k < board.size(); k++) selections[k] = false;
