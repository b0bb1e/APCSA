...
public class Shuffler {
	...
	public static void main(String[] args) {
		...
		for (int value : values1) System.out.print(value + " ");
	...
	public static void perfectShuffle(int[] values) {
		...
		int spot = 0;
		for (int i = 0, n = values.length / 2; i < n; i++) {
			shuffled[i] = values[spot];
			spot += 2;
		}
		spot = 1;
		for (int i = values.length / 2, n = values.length; i < n; i++) {
			shuffled[i] = values[spot];
			spot += 2;
		}
		for (int i = 0, n = shuffled.length; i < n; i++) values[i] = shuffled[i];
	}
